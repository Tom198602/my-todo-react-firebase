...
[Portfolio Page](https://react-ts-firebase-todo-d5d7f.web.app/portfolio)
...
- 作品紹介用の `/portfolio` ページ
- Vitest によるタスク集計ロジックの単体テスト
...
src/
  ...
  lib/          Pure functions and unit tests
...
- `src/App.tsx`: ログイン状態によるルーティングと公開用 `/portfolio`
...
- `src/lib/taskUtils.ts`: タスクの集計とフィルター処理
- `src/pages/Portfolio.tsx`: 採用担当者向けの作品紹介ページ

## What I Focused On

このアプリは小さな Todo アプリですが、採用担当者が見やすいように「実務で必要な状態管理」を入れています。Firebase との通信は `useTasks` にまとめ、画面表示は小さなコンポーネントに分けることで、処理の流れを説明しやすい構成にしています。タスクの集計・フィルター処理は `taskUtils` に切り出し、単体テストで確認できるようにしました。
